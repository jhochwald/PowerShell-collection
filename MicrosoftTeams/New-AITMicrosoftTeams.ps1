...
<#
      ...
      Team display name. Team Name Characters Limit is 256.
...
begin
{
   ...
   {
      Write-Error -Message 'You have selected muliple authentication methods. This is not valid' -Exception 'Muliple authentication methods selected' -Category AuthenticationError -ErrorAction Stop
   }
   ...
   if ($pscmdlet.ShouldProcess($DisplayName, 'Create'))
   {
      try
      {
         ...
         if (-not ($mfa))
         {
            ...
            $null = (Connect-MicrosoftTeams -Credential $msTeamsCreds -Confirm:$false -ErrorAction Stop)
         ...
         else
         {
            ...
            if ($AddCreatorAsMember -eq $false)
            {
               ...
               Remove-TeamUser -GroupId $NewTeam.GroupId -User $msTeamsCreds.UserName
            }
            ...
            if ($Owner)
            {
               foreach ($Admin in $Owner)
               {
                  try
                  {
                     $null = (Add-TeamUser -GroupId $NewTeam.GroupId -User $Admin -Role Owner -ErrorAction Stop)
                  }
                  catch
                  {
            ...
            if ($User)
            {
               foreach ($Member in $User)
               {
                  try
                  {
                     $null = (Add-TeamUser -GroupId $NewTeam.GroupId -User $Member -Role Member -ErrorAction Stop)
                  }
                  catch
                  {
