...
try
{
   ...
   if (-not ((Get-ItemPropertyValue -LiteralPath $RegistryPath -Name 'Microsoft.Office.Excel.CopilotExperiment' -ErrorAction Stop) -eq 'true'))
   {
      exit 1
   }
   ...
   if (-not ((Get-ItemPropertyValue -LiteralPath $RegistryPath -Name 'Microsoft.Office.OneNote.Copilot' -ErrorAction Stop) -eq 'true'))
   {
      exit 1
   }
   ...
   if (-not ((Get-ItemPropertyValue -LiteralPath $RegistryPath -Name 'Microsoft.Office.PowerPoint.CopilotExperiment' -ErrorAction Stop) -eq 'true'))
   {
      exit 1
   }
