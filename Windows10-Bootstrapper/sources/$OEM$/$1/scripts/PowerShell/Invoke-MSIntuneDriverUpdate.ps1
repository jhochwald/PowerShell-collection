...
begin {
   ...
   [string]$TempDirectory = (Join-Path -Path 'c:\install' -ChildPath '\SCConfigMgr')
   [string]$LogDirectory = (Join-Path -Path 'c:\temp' -ChildPath '\SCConfigMgr')

   # Create Temp Folder 
   if ((Test-Path -Path $TempDirectory) -eq $false) {
      $null = (New-Item -Path $TempDirectory -ItemType Dir -Force)
   }

   # Create Logs Folder 
   if ((Test-Path -Path $LogDirectory) -eq $false) {
      $null = (New-Item -Path $LogDirectory -ItemType Dir -Force)
   }

   $null = (Push-Location -Path 'c:\install')
   ...
   #>
      ...
      process {
         ...
         try {
            $null = (Add-Content -Value $LogText -LiteralPath $LogFilePath -Force -ErrorAction Stop)
         }
         catch {
   ...
   $script:LenovoXMLFile = [string]($LenovoXMLSource | Split-Path -Leaf)
      ...
      switch ($ComputerManufacturer ) {
         'Hewlett-Packard' {
            if ((Test-Path -Path $TempDirectory\$HPCabFile) -eq $false) {
               ...
               try {
                  $null = (Start-BitsTransfer -Source $HPXMLCabinetSource -Destination $TempDirectory)
         ...
         'Dell' {
            if (-not (Test-Path -Path $TempDirectory\$DellCabFile)) {
               ...
               try {
                  $null = (Start-BitsTransfer -Source $DellXMLCabinetSource -Destination $TempDirectory -ErrorAction Stop)
